...
class CampusMapApp:
    def __init__(self, root, graph, all_edges):
        ...
        # --- SIMPLE HEADER (UI ADDITION) ---
        header_frame = tk.Frame(self.root, bg="#1565C0", padx=15, pady=10)
        ...
        tk.Label(
            header_frame,
            text="Find the shortest route between campus locations",
            bg="#1565C0",
            fg="#E3F2FD",
            font=("Arial", 10)
        ).pack(anchor="w")
        ...
        self.via_combo.grid(row=0, column=3, padx=5)
        ...
        self.end_combo.grid(row=0, column=5, padx=5)
        ...
        find_btn.grid(row=0, column=6, padx=7)
        ...
        reset_btn.grid(row=0, column=7, padx=7)

        # Status Message Label
        self.result_label = tk.Label(
            control_frame,
            text="Select points via dropdown or click nodes directly on the map.",
            bg="white",
            fg="#37474F",
            font=("Arial", 10, "bold"),
            justify=tk.LEFT,
        ...
        self.canvas.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 5))
        self.canvas.bind("<Button-1>", self.on_canvas_click)

        # --- SIMPLE MAP LEGEND (UI ADDITION) ---
        legend_label = tk.Label(
            self.root,
            text=(
                "Legend:  Blue = Location   Green = Start   "
                "Yellow = Via   Orange = End   Red = Route   "
        ...
        legend_label.pack(fill=tk.X, padx=10, pady=(0, 6))
    ...
    def on_canvas_click(self, event):
        ...
        if clicked_node:
            if self.click_step == 0:
                ...
                self.click_step = 1
                self.result_label.config(
                    text=f"Start: '{clicked_node}'. Now click an End node.",
                    fg="#1976D2"
                )
    ...
    def find_and_draw_route(self):
        ...
        if start == end or (
            via != "None" and (start == via or end == via)
        ):
        ...
        if via != "None":
            ...
            if (
                ...
                self.result_label.config(
                    text="No valid path passing through the selected middle stop!",
                    fg="red"
                )
                return
            ...
            if dist == float("inf") or not path:
                self.result_label.config(
                    text="No path exists between selected points!",
                    fg="red"
                )
                return
        ...
        for edge in self.all_edges:
            if (
                ...
                warnings.append(
                    f"⚠️ [{edge['u']} ↔ {edge['v']} ({status})]: {info}"
                )

        # Display Total Distance + Walk Time
        result_msg = (
            f"Total Distance: {dist} meters "
